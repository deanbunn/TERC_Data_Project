...
def handler(event,context):
    ...
    if(event['id'] and event['id'].isdigit() and event['rptdate'] and event['rptdate'].isdigit() and len(event['rptdate']) == 8):
        ...
        if(n_stationid == 1):
            sp_cmd = "CALL sp_report_ns_station_granrange_cascade('"
            terc_station = "Cascade" 
            
        if(n_stationid == 2):
            sp_cmd = "CALL sp_report_ns_station_granrange_dollarpoint('"
            terc_station = "Dollar Point"
            
        if(n_stationid == 3):
            sp_cmd = "CALL sp_report_ns_station_granrange_glenbrook('"
            terc_station = "Glenbrook"
            
        if(n_stationid == 4):
            sp_cmd = "CALL sp_report_ns_station_granrange_homewood('"
            terc_station = "Homewood"
        
        if(n_stationid == 5):
            sp_cmd = "CALL sp_report_ns_station_granrange_meeks('"
            terc_station = "Meeks"
        
        if(n_stationid == 6):
            sp_cmd = "CALL sp_report_ns_station_granrange_rubicon('"
            terc_station = "Rubicon"
        
        if(n_stationid == 7):
            sp_cmd = "CALL sp_report_ns_station_granrange_sandharbor('"
            terc_station = "Sand Harbor"
             
        if(n_stationid == 8):
            sp_cmd = "CALL sp_report_ns_station_granrange_tahoevista('"
            terc_station = "Tahoe Vista"
            
        if(n_stationid == 9):
            sp_cmd = "CALL sp_report_ns_station_granrange_tahoecity('"
            terc_station = "Tahoe City"
        
        if(n_stationid == 10):
            sp_cmd = "CALL sp_report_ns_station_granrange_camprichardson('"
            terc_station = "Camp Richardson"

        if(n_stationid == 11):
            sp_cmd = "CALL sp_report_ns_station_granrange_timbercove('"
        ...
        if(len(sp_cmd) > 1):
            ...
            sp_cmd += sp_rpt_date + " 00:00:00','" + sp_rpt_end + " 23:59:59')"  
